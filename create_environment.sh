...
read user_surname;
# create parent directory called submission_reminder_Janviere
...
#I am going to create files inside these subdirectories

if [ -d "submission_reminder_Janviere" ]; then
        touch $parent_dir/app/reminder.sh
	chmod +x $parent_dir/app/reminder.sh
fi

if [ -d "submission_reminder_Janviere" ]; then
        touch $parent_dir/modules/fuctions.sh
	chmod +x $parent_dir/modules/fuctions.sh
fi
...
# Entering reminder.sh information about those who submitted or not
echo "#!/bin/bash
...
}" > "$parent_dir/modules/fuctions.sh"
...
Jean, Shell Basics, not submitted" > "$parent_dir/assets/submissions.txt"
...
# Start reminder app.
./app/reminder.sh" > $parent_dir/startup.sh 
